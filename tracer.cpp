...
struct V {
  ...
  V operator !() { return *this *(1 / sqrtf(*this % *this)); }
};
...
int M(V o, V d, V &h, V &n) {
  int m, s = 0;
  float t = 0, c;
  for (; t < 100; t += c)
    if ((c = S(h = o + d * t, m)) < .01 || ++s > 99)
      return n = !V(S(h + V(.01, 0), s) - c, S(h + V(0, .01), s) - c,
                    S(h + V(0, 0, .01), s) - c),
             m;
  return 0;
}
...
bool raySphereIntersect(const V& orig, const V& dir, const float& radius, float& t0, float& t1)
{
...
V computeIncidentLight(V sunDirection, V orig, V dir, float tmin, float tmax)
{
